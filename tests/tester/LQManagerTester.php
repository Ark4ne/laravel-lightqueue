...
abstract class LQManagerTester extends TestCase
{
	...
	public function testExceptionJob()
	{
		...
		$this->setExpectedException('Ark4ne\LightQueue\Exception\LightQueueException',
									'JobError not implement LightQueueCommandInterface');
		Artisan::call('lq:exec', [], $output);

		$output->fetch();
	}

	public function testDestructJob()
	{
		LightQueue::instance()->push('job', ['data' => 'test'], $this->queueName);
		$this->assertEquals(1, $this->manager->queueSize($this->queueName));
		$this->assertTrue(is_bool($this->manager->jobDestruct($this->queueName)));
		LightQueue::instance()->pop($this->queueName);
	}
	...
	public function testNullQueueName()
	{
		...
		$this->assertEquals((object)[
			...
			'data'  => 'DataQueueNull',
			'queue' => 'process'],
							$this->manager->nextQueue());

		$this->assertEquals(0, $this->manager->queueSize(null));
	}
